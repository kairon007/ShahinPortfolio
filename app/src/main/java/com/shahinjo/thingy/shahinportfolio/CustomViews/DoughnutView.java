...
public class DoughnutView extends ViewGroup {
    ...
    private final Property<DoughnutView, Float> percentProperty = new Property<DoughnutView, Float>(Float.class, "Percent") {
        @Override
        public Float get(DoughnutView fd) {
            return fd.getPercent();
        }

        @Override
        public void set(DoughnutView fd, Float value) {
            fd.setPercent(value);
        }
    };
    // OriginAngle
    ...
    private final Property<DoughnutView, Float> originAngleProperty = new Property<DoughnutView, Float>(Float.class, "OriginAngle") {
        @Override
        public Float get(DoughnutView fd) {
            return fd.getOriginAngle();
        }

        @Override
        public void set(DoughnutView fd, Float value) {
            fd.setOriginAngle(value);
        }
    };

    public DoughnutView(Context ctx) {
        super(ctx);
        init();
    }

    public DoughnutView(Context ctx, AttributeSet attrs) {
        super(ctx, attrs);

        TypedArray a = ctx.getTheme().obtainStyledAttributes(attrs, R.styleable.DoughnutView, 0, 0);
    ...
    private void init() {
        ...
        tailAnimator.setDuration(DoughnutView.ANIM_DURATION_LONG);
    ...
    public void animateSetPercent(float percent) {
        ...
        setPercent(percent);
        animateRing(old, getPercent(), DoughnutView.ANIM_DURATION_LONG);
        ...
        if (headAnimator.isRunning() && tailAnimator.isRunning()) {
            ...
            headAnimator.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationRepeat(Animator animation) {
                    ...
                    headAnimator.setDuration(DoughnutView.ANIM_DURATION_DEFAULT);
    ...
    public void startAnimateLoading() {
        ...
        headAnimator.setDuration(DoughnutView.ANIM_DURATION_LONG);
    ...
    public void startAnimateLoadingCount(int count) {
        headAnimator.setFloatValues(.1f, 66.f, .1f);
        headAnimator.setDuration(DoughnutView.ANIM_DURATION_LONG);
        headAnimator.setRepeatCount(count);
